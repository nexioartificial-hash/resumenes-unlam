...
CONTENT_ITEM_ID = "7b0c19ce-9a47-48c6-b31e-20a032165d2f"

WATERMARK      = '@RESUMENES.UNLAM'
TITLE_PARTS    = {'RESUMEN DE', 'CONTABILIDAD', 'CURSO DE INGRESO'}
INLINE_BULLETS = {'●': '- ', '○': '  - ', '■': '    - '}  # inline bullet → markdown
...
def count_content_words(text):
    ...
    for line in text.split('\n'):
        ...
        if s in TITLE_PARTS: continue
        for w in s.split():
            if w not in INLINE_BULLETS:  # excluir los chars de bullet
                words.append(w)
    return len(words)


# ── Tamaño de fuente por línea ────────────────────────────────────────────────
...
for page_num in range(len(doc)):
    ...
    while i < len(raw_lines):
        ...
        # ── Omitir: página, watermark, título de portada ──────────────────────
        ...
        for bullet_char, marker in INLINE_BULLETS.items():
            ...
            # ── Heading de módulo (MÓDULO N: …) ──────────────────────────────
            if re.match(r'^MÓDULO \d+:', s):
                heading = s
                # Detectar continuación de título en la siguiente línea
                j = i + 1
            ...
            # ── Sub-heading por tamaño (numbered: 1. Texto, size ≥ 12.5) ─────

...
const SUBJECTS_WITH_MAP = ['filosofia']
      ...
      <div className="bg-white border-b border-tinta/10 px-5 py-3 flex items-center gap-4 shrink-0">
        ...
        <div className="hidden sm:flex items-center gap-4 text-xs text-tinta/40">
          <span className="flex items-center gap-1.5">
            <span className="w-3 h-3 rounded-full inline-block" style={{ background: '#0F3F26' }} />
            Dominado (≥70%)
          </span>
          <span className="flex items-center gap-1.5">
            <span className="w-3 h-3 rounded-full inline-block" style={{ background: '#F5D63E' }} />
            En progreso
          </span>
          <span className="flex items-center gap-1.5">
            <span className="w-3 h-3 rounded-full inline-block" style={{ background: '#C8332A' }} />
            A reforzar
          </span>
          <span className="flex items-center gap-1.5">
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-full gap-4 p-8">
            ...
            <p className="text-tinta/40 text-sm text-center max-w-sm">
              ...
              Por ahora está disponible solo para Filosofía.
            </p>
            <button
              onClick={() => router.back()}
          ...
          <KnowledgeMap mastery={mastery} slug={slug} />
        )}
      </div>
    </div>
  )
